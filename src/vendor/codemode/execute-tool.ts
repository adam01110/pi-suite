...
import { type ExecutionResult, executeCode } from "./sandbox.js";
import {
	createToolBindings,
	type ToolBindingsOptions,
...
export interface ExecuteToolOptions {
	...
	bindingsOptions: Omit<
		ToolBindingsOptions,
		"extensionContext" | "onUpdate" | "signal" | "toolCallId"
	>;
...
Return a value to include it in the result. Type errors are returned for correction.`,
		...
		renderCall(
			...
			try {
				...
				let text = Array.isArray(highlighted)
					? highlighted.join("\n")
					: String(highlighted);
