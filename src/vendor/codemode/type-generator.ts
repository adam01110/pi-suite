...
`;
	...
	for (const server of servers) {
		...
		parts.push(
			`  ${server.namespace}: ${serverInterfaceName(server.namespace)};`,
		);
	}
	parts.push(`}`);
	parts.push(``);
	...
	for (const server of servers) {
		...
		for (const tool of server.tools) {
			...
			parts.push(
				`  ${safeName}(args${argsOptional ? "?" : ""}: ${paramsType}): Promise<string>;`,
			);
		}
		parts.push(`}`);
		parts.push(``);
...
export function generateMcpSummaryForPrompt(servers: McpServerInfo[]): string {
	...
	lines.push(``);
	lines.push(
		`The following MCP servers are available as typed namespaces on \`tools\`.`,
	);
	lines.push(
		`Use \`describe_tools\` to browse tools in a namespace and see their parameters.`,
	);
	lines.push(
		`Use \`search_tools\` to find tools by keyword across all servers.`,
	);
	lines.push(``);
	...
	for (const server of servers) {
		...
		if (count === 0) {
			lines.push(
				`- **tools.${server.namespace}** — ${server.serverName} (connect on first call)`,
			);
		} else {
			lines.push(
				`- **tools.${server.namespace}** — ${server.serverName} (${count} tools)`,
			);
		}
	}
...
export function generateToolSignature(
	...
	lines.push(
		`tools.${namespace}.${toolName}(args${argsOptional ? "?" : ""}: ${paramsType}): Promise<string>`,
	);
	return lines.join("\n");
}
...
function jsonSchemaToTypeString(schema: unknown, indent: string = ""): string {
	...
	if (Array.isArray(s.type)) {
		return s.type
			.map((t) => jsonSchemaToTypeString({ type: t }, indent))
			.join(" | ");
	}
