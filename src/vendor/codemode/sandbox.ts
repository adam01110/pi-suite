...
import { stop as stopEsbuild, transformSync } from "esbuild";
...
function truncateFromTail(rawText: string): TruncationResult {
	...
	for (
		...
		i >= 0 && kept.length < MAX_OUTPUT_LINES;
		i--
	) {
		const lineBytes =
			Buffer.byteLength(lines[i], "utf-8") + (kept.length > 0 ? 1 : 0);
		if (keptBytes + lineBytes > MAX_OUTPUT_BYTES) {
			...
			if (kept.length === 0) {
				const partialLine = truncateStringToBytesFromEnd(
					lines[i],
					MAX_OUTPUT_BYTES,
				);
 ...
 */
function createTruncatedString(
	fullText: string,
...
function createTruncating$(
	...
	const wrapped = (pieces: TemplateStringsArray, ...args: any[]) => {
		const proc = base$(pieces, ...args) as zx.ProcessPromise;

		// Wrap .then to intercept the ProcessOutput and truncate it
		const origThen = proc.then.bind(proc);
		// biome-ignore lint/suspicious/noThenProperty: Wrap zx's promise to truncate output.
		proc.then = (<T1 = zx.ProcessOutput, T2 = never>(
		...
		if (signal) {
			racePromises.push(
				new Promise<never>((_, reject) => {
					...
					abortHandler = () => reject(new Error("Execution cancelled"));
