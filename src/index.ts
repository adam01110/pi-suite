...
import headerRefresh from "./glue/header-refresh.js";
...
import { formatSuiteStatus, loadModules, type SuiteModule, statusNotifyLevel } from "./registry.js";
...
const IGNORED_NOTIFICATION_PREFIXES = {
  computerUse: new Set(["@agent-sh/computer-use-linux: native Pi tools are active."]),
...
export default async function piSuite(pi: ExtensionAPI): Promise<void> {
  ...
  const modules: readonly SuiteModule[] = [
    ...
    {
      id: "footer",
      factory: suppressCommands(upstreamFactory("pi-footer/src/index.js"), BLOCKED_COMMANDS.footer),
      optional: true,
    },
    ...
    {
      id: "mcp",
      factory: suppressCommands(upstreamFactory("pi-mcp-adapter"), BLOCKED_COMMANDS.mcp),
      optional: true,
    },
    {
      id: "ask-user",
    ...
    {
      id: "web-access",
      factory: suppressCommands((api) => webAccess(api, tools), BLOCKED_COMMANDS.webAccess),
      optional: true,
    },
  ...
  pi.registerCommand("suite", {
    ...
    handler: async (_args, ctx) => {
      ctx.ui.notify(formatSuiteStatus(results), statusNotifyLevel(results));
    },
  });
}
