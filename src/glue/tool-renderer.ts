...
function resultText(result: Parameters<NonNullable<AnyToolDefinition["renderResult"]>>[0]): string {
...
function lspCallText(
  ...
  const path = typeof input.path === "string" ? ` ${theme.fg("muted", input.path)}` : "";
...
export function batchValidationText(raw: string): string | undefined {
  ...
  const issue = raw.match(/\n\s*-\s*(.+?)\n\nReceived arguments:/s)?.[1]?.trim();
...
function compactBatchValidation(definition: AnyToolDefinition): AnyToolDefinition {
  return {
    ...definition,
    renderResult(result, options, theme, context) {
      const firstText = result.content.find((part) => part.type === "text");
      if (!firstText?.text.startsWith(BATCH_VALIDATION_PREFIX) && definition.renderResult) {
...
function compactLspDefinition(definition: AnyToolDefinition): AnyToolDefinition {
  return {
    ...definition,
    renderShell: "self",
    ...
    renderResult(result, { isPartial, expanded }, theme, context) {
      ...
      const raw = definition.name === "lsp_diagnostics" ? diagnosticsDisplayText(text) : text;
      if (context.isError)
        return new Text(theme.fg("error", raw.split("\n")[0] || "LSP request failed"), 0, 0);
...
export default async function toolRendererAdapter(
  ...
  const originals = new Map(["grep", "find"].map((name) => [name, tracker.get(name)] as const));
  ...
  const onToolResult = pi.on.bind(pi) as unknown as (
    event: "tool_result",
    handler: (event: ToolResultEvent) => { content: ToolResultEvent["content"] } | void,
  ) => void;
  ...
  pi.registerMessageRenderer("pi-lsp-diagnostics", (message, _options, theme) => {
    const details = message.details as { path?: unknown; summary?: unknown } | undefined;
    const path = typeof details?.path === "string" ? ` ${theme.fg("muted", details.path)}` : "";
    const summary =
      typeof details?.summary === "string" ? diagnosticsDisplayText(details.summary) : "";
    ...
    container.addChild(
      new Text(summary ? `${title}\n${theme.fg("toolOutput", summary)}` : title, 0, 0),
    );
    container.addChild(border());
    return container;
  });
